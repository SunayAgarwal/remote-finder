...
//receive code as HEX type, receive code as uint8_t type
...
String recievedMessage = "Packet received";
uint8_t recievedMessageInt = recievedMessage.toInt();
...
void loop() {
  ...
  if (packetSize) {
    ...
    Serial.println(packet2);                 //packet is new MAC address
    dev1.addAddress(packet2);
    UDP.beginPacket(UDP.remoteIP(), UDP.remotePort());
    UDP.write(recievedMessageInt);
    UDP.endPacket();
